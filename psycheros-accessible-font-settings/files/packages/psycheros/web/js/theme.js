...
function normalizeFontPreset(preset) {
  return FONT_PRESETS[preset] ? preset : DEFAULT_THEME.fontPreset;
}

function clampFontSize(size) {
  const parsed = Number(size);
  if (!Number.isFinite(parsed)) return DEFAULT_THEME.fontSize;
  return Math.max(12, Math.min(28, Math.round(parsed)));
}

function applyTypography(theme) {
  const root = document.documentElement;
  const fontPreset = normalizeFontPreset(theme.fontPreset);
  const fontSize = clampFontSize(theme.fontSize);
  const fontStack = FONT_PRESETS[fontPreset].stack;

  root.style.setProperty('--font-sans', fontStack);
  root.style.setProperty('--font-size-xs', `${Math.round(fontSize * 0.6875)}px`);
  root.style.setProperty('--font-size-sm', `${Math.round(fontSize * 0.8125)}px`);
  root.style.setProperty('--font-size-md', `${Math.round(fontSize * 0.9375)}px`);
  root.style.setProperty('--font-size-base', `${fontSize}px`);
  root.style.setProperty('--font-size-lg', `${Math.round(fontSize * 1.125)}px`);
  root.style.setProperty('--font-size-xl', `${Math.round(fontSize * 1.25)}px`);
}
...
function setFontPreset(preset) {
  currentTheme.fontPreset = normalizeFontPreset(preset);
  saveTheme(currentTheme);
  applyTheme(currentTheme);
}

function setFontSize(size) {
  currentTheme.fontSize = clampFontSize(size);
  saveTheme(currentTheme);
  applyTheme(currentTheme);
}
