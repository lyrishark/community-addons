...
Settings > Vision provides four tabs:
...
- Expression detection uses a hybrid stream-and-settle model. Recent-turn intent
  ...
  story, and topic emotion words are dampened so they do not automatically
  become the entity's displayed feeling.
...
- Frame styles include transparent, square, circle, and accent treatment; the
  default recommends transparent PNG/WebP assets so sprites can sit over chat
  backgrounds cleanly
- Optional checkerboard cleanup removes common fake-transparency backgrounds
  from generated/imported sprites
- Optional subtitle shows the current emotion label under the sprite; hover
