...
import {
  hashScreenFrame,
  SCREEN_PRESENCE_MAX_FRAME_BYTES,
  type ScreenPresenceService,
} from "./screen-presence.ts";
...
export interface RouteContext {
  ...
  getScreenPresence: () => ScreenPresenceService;
...
export async function handleChatRetry(
  ...
  // Retrieve the requested user message, or fall back to the last user message.
    ...
    return new Response(
      JSON.stringify({
        error: body.userMessageId
          ? "Requested user message not found"
 ...
 */
export async function handleGetExpressionDisplaySettings(
  ctx: RouteContext,
): Promise<Response> {
...
/**
 * Handle POST /api/expression-sprites/settings - Save display/fallback settings.
 *
 * Sprite asset records are server-owned and are intentionally preserved here so
 * a stale settings form cannot orphan files.
 */
export async function handleSaveExpressionDisplaySettings(
  ctx: RouteContext,
  request: Request,
): Promise<Response> {
  try {
    const current = await loadExpressionDisplaySettings(ctx.dataRoot);
    const body = await request.json();
    const updated = normalizeExpressionDisplaySettings({
      ...current,
    ...
    console.error(
      "[Routes] Failed to save expression display settings:",
      error,
    );
    return new Response(
...
/**
 * Handle POST /api/expression-sprites/:label - Upload one sprite for a label.
 */
export async function handleUploadExpressionSprite(
  ctx: RouteContext,
  labelParam: string,
  request: Request,
): Promise<Response> {
  try {
    const label = normalizeExpressionLabel(decodeURIComponent(labelParam));
    if (!label) {
      return new Response(
    ...
    const settings = await loadExpressionDisplaySettings(ctx.dataRoot);
    const filename = buildExpressionSpriteFilename(label, file.name);
    const cleanup = processExpressionSpriteBytes(
      file.name,
      new Uint8Array(await file.arrayBuffer()),
      settings.cleanupCheckerboardBackgrounds,
    );
    const bytes = cleanup.bytes;
    const previous = settings.sprites[label];
    const spritesDir = getExpressionSpritesDir(ctx.dataRoot);
    await Deno.mkdir(spritesDir, { recursive: true });
    await Deno.writeFile(
      getExpressionSpritePath(ctx.dataRoot, filename),
      bytes,
    );
    ...
    if (previous) {
      try {
        await Deno.remove(
          getExpressionSpritePath(ctx.dataRoot, previous.filename),
        );
      } catch {
    ...
    const asset = createExpressionSpriteAsset(
      label,
      filename,
      file.name,
      bytes,
    );
    settings.sprites[label] = asset;
    ...
    return new Response(
      JSON.stringify({
        success: true,
        asset: { ...asset, url: expressionSpriteUrl(asset.filename) },
...
/**
 * Handle POST /api/expression-sprites/import - Import a SillyTavern-style ZIP pack.
 */
export async function handleImportExpressionSpritePack(
  ctx: RouteContext,
  request: Request,
): Promise<Response> {
  try {
    ...
    for (const entry of Object.values(zip.files)) {
      ...
      const label = matchExpressionLabelFromFilename(
        originalName,
        settings.labels,
      );
      if (!label) {
        skipped++;
        if (unmatched.length < 20) unmatched.push(originalName);
        continue;
      }
      ...
      const bytes = cleanup.bytes;
      if (cleanup.changed) cleaned++;
      const filename = buildExpressionSpriteFilename(label, originalName);
      const previous = settings.sprites[label];
      await Deno.writeFile(
        getExpressionSpritePath(ctx.dataRoot, filename),
        bytes,
      ...
      settings.sprites[label] = createExpressionSpriteAsset(
        label,
        filename,
        originalName,
        bytes,
...
/**
 * Handle DELETE /api/expression-sprites/:label - Remove one label's sprite.
 */
export async function handleDeleteExpressionSprite(
  ctx: RouteContext,
  labelParam: string,
): Promise<Response> {
  try {
    const label = normalizeExpressionLabel(decodeURIComponent(labelParam));
    const settings = await loadExpressionDisplaySettings(ctx.dataRoot);
    const existing = settings.sprites[label];
    if (!existing) {
      return new Response(
    ...
    try {
      await Deno.remove(
        getExpressionSpritePath(ctx.dataRoot, existing.filename),
      );
    } catch {
      // Missing files should not keep stale settings around.
    }
...
/**
 * Handle GET /expression-sprites/:filename - Serve uploaded expression sprite files.
 */
export async function handleServeExpressionSprite(
  ctx: RouteContext,
  filenameParam: string,
): Promise<Response> {
  const filename = decodeURIComponent(filenameParam);
  if (!isSafeExpressionSpriteFilename(filename)) {
  ...
  try {
    ...
    return new Response(data, {
      headers: {
        "Content-Type": getExpressionSpriteMimeType(filename),
...
function processExpressionSpriteBytes(
  originalName: string,
  bytes: Uint8Array,
  cleanupEnabled: boolean,
): { bytes: Uint8Array; changed: boolean } {
  if (!cleanupEnabled || !shouldAttemptCheckerboardCleanup(originalName)) {
    return { bytes, changed: false };
  }

  const cleanup = removeCheckerboardBackgroundFromPng(bytes);
  return { bytes: cleanup.bytes, changed: cleanup.changed };
}

function createExpressionSpriteAsset(
  label: string,
  filename: string,
  originalName: string,
  bytes: Uint8Array,
): ExpressionSpriteAsset {
  return {
    label,
    filename,
    originalName,
    mimeType: getExpressionSpriteMimeType(filename),
...
// =============================================================================
// Screen Presence API Routes
// =============================================================================

/**
 * Handle GET /api/screen-presence/status - Return my current screen observation state.
 */
export function handleGetScreenPresenceStatus(ctx: RouteContext): Response {
  return new Response(
    JSON.stringify(ctx.getScreenPresence().getSnapshot()),
    { headers: JSON_HEADERS },
  );
}

/**
 * Handle POST /api/screen-presence/session - Start or stop a screen observation session.
 */
export async function handleScreenPresenceSession(
  ctx: RouteContext,
  request: Request,
): Promise<Response> {
  try {
    ...
    if (typeof body.active !== "boolean") {
      return new Response(
        JSON.stringify({ error: "active must be a boolean" }),
        { status: 400, headers: JSON_HEADERS },
      );
    }
    ...
    const sourceLabel = typeof body.sourceLabel === "string"
      ? body.sourceLabel
      : undefined;
    const snapshot = body.active
      ? ctx.getScreenPresence().start(sourceLabel)
      : ctx.getScreenPresence().stop();
...
/**
 * Handle POST /api/screen-presence/frame - Accept a transient screen frame.
 *
 * Frames are captioned when Vision captioning is enabled, then discarded. The
 * in-memory state keeps only the latest compact summary and status.
 */
export async function handleScreenPresenceFrame(
  ctx: RouteContext,
  request: Request,
): Promise<Response> {
  try {
    ...
    if (!["image/jpeg", "image/png", "image/webp"].includes(mediaType)) {
      return new Response(
        JSON.stringify({ error: `Unsupported frame type: ${mediaType}` }),
        { status: 400, headers: JSON_HEADERS },
      );
    }

    const bytes = new Uint8Array(await file.arrayBuffer());
    if (bytes.byteLength > SCREEN_PRESENCE_MAX_FRAME_BYTES) {
      return new Response(
        JSON.stringify({ error: "Frame is too large" }),
    ...
    const sourceLabel = typeof sourceLabelRaw === "string"
      ? sourceLabelRaw
      : undefined;
    const capturedAt = typeof capturedAtRaw === "string"
      ? capturedAtRaw
      : undefined;
    const forceCaption = formData.get("forceCaption") === "true";
    const hash = await hashScreenFrame(bytes);
    const screenPresence = ctx.getScreenPresence();
    const decision = screenPresence.acceptFrame({
      hash,
      sourceLabel,
      capturedAt,
      forceCaption,
    ...
    if (decision.shouldCaption) {
      const captioning = ctx.getImageGenSettings().captioning;
      if (!captioning?.enabled || !captioning.provider) {
        screenPresence.markCaptionUnavailable();
      } else {
        try {
          const caption = await captionImageDual(
            uint8ToBase64(bytes),
          ...
          screenPresence.finishCaption(hash, caption);
        } catch (error) {
          screenPresence.failCaption(hash, error);
        }
      }
    }
    ...
    return new Response(
      JSON.stringify({
        ...screenPresence.getSnapshot(),
        frameAccepted: true,
        captionDecision: decision.reason,
      }),
      { headers: JSON_HEADERS },
    );
  } catch (error) {
    console.error(
      "[ScreenPresence] Failed to process frame:",
      error instanceof Error ? error.message : String(error),
    );
    return new Response(
      JSON.stringify({ error: "Failed to process screen frame" }),
      { status: 500, headers: JSON_HEADERS },
    );
  }
}
...
/**
 * Handle GET /fragments/settings/vision/expressions - Expressions tab content.
 */
export async function handleVisionExpressionsFragment(
  ctx: RouteContext,
): Promise<Response> {
  const settings = await loadExpressionDisplaySettings(ctx.dataRoot);
  const html = renderVisionExpressionsTab(settings);
  return new Response(html, {
    headers: { "Content-Type": "text/html; charset=utf-8" },
  });
}
