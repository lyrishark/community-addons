...
import { classifyExpressionText } from "../expression/mod.ts";
import {
...
function renderExpressionStateChip(content: string): string {
  ...
  const seed =
    `<span class="expression-state-seed" hidden data-expression-label="${
      escapeHtml(String(state.label))
    }" data-expression-confidence="${
      escapeHtml(String(state.confidence))
    }" data-expression-rationale="${escapeHtml(state.rationale)}"></span>`;

  if (state.label === "neutral") return seed;

  return `${seed}<span class="expression-state-display expression-state-display--label expression-state-chip" data-expression-label="${
    escapeHtml(String(state.label))
  }" data-expression-confidence="${
    escapeHtml(String(state.confidence))
  }" data-expression-rationale="${escapeHtml(state.rationale)}" title="${
    escapeHtml(title)
  }">${escapeHtml(label)}</span>`;
}

function formatExpressionLabel(label: string): string {
  return String(label)
    .replace(/[-_]+/g, " ")
...
<body>
  ...
  <div class="app">
    ...
    <div class="main">
      ...
      <div class="expression-stage" id="expression-stage" aria-live="polite" hidden></div>
      <button
...
export function renderMessages(
  ...
  const missingAssistantUserId = latestVisibleMessage?.role === "user" &&
      !latestVisibleMessage.pulseId && !latestVisibleMessage.pulseName
  ...
  return visibleMessages
    .map((m) =>
      renderMessage(
        ...
        metricsMap?.get(m.id),
        displayNames,
        Boolean(missingAssistantUserId && m.id === missingAssistantUserId),
      )
    )
    .join("");
}
...
export function renderMessage(
  ...
  metrics?: TurnMetrics,
  displayNames?: { entityName: string; userName: string },
  allowMissingAssistantRegenerate = false,
): string {
  ...
  if (voicedMsg.role === "user") {
    ...
    return renderUserMessage(
      ...
      msg.isVoice,
      allowMissingAssistantRegenerate,
    );
...
export function renderUserMessage(
  ...
  const editBtn = messageId
    ...
    : "";
  const regenerateBtn = allowMissingAssistantRegenerate && messageId
    ? `<button class="msg-regenerate-btn" onclick="Psycheros.regenerateMissingAssistantTurn('${
      escapeHtml(messageId)
    }')" title="Regenerate missing response" aria-label="Regenerate missing response">
...
/**
 * Render the Expressions tab panel content.
 */
export function renderVisionExpressionsTab(
  settings: ExpressionDisplaySettings,
): string {
  const oobTabs = renderVisionTabActiveState("expressions");
  const configuredCount = Object.keys(settings.sprites).length;
  const cards = settings.labels.map((label) => {
    const asset = settings.sprites[label];
    const displayLabel = formatExpressionLabel(label);
    const preview = asset
      ? `<img src="${escapeHtml(expressionSpriteUrl(asset.filename))}" alt="${
        escapeHtml(displayLabel)
      } sprite" class="expression-settings-thumb" loading="lazy">`
      : `<div class="expression-settings-empty" aria-hidden="true">${
        escapeHtml(displayLabel.slice(0, 2).toUpperCase())
      }</div>`;
    const meta = asset
    ...
    const deleteButton = asset
      ? `<button class="btn btn--sm btn--danger" type="button" onclick="deleteExpressionSprite('${label}')">Delete</button>`
      : "";

    return `<div class="expression-settings-card" data-expression-label="${label}">
      <div class="expression-settings-preview expression-settings-preview--${
      escapeHtml(settings.frameStyle)
    }">
        ${preview}
      </div>
      <div class="expression-settings-card-body">
        <div class="expression-settings-label">${escapeHtml(displayLabel)}</div>
        <div class="expression-settings-meta" title="${
      escapeHtml(meta)
    }">${meta}</div>
      </div>
      <div class="expression-settings-actions">
        <label class="btn btn--sm" style="position:relative;overflow:hidden;cursor:pointer;">
          Choose
          <input type="file" id="expression-sprite-file-${label}" accept="image/png,image/jpeg,image/gif,image/webp" style="position:absolute;inset:0;opacity:0;cursor:pointer;" onchange="uploadExpressionSprite('${label}')">
        </label>
        ${deleteButton}
      </div>
    </div>`;
  ...
  <style>
    ...
    .expression-settings-card { display: grid; grid-template-columns: 52px minmax(0, 1fr); grid-template-rows: auto auto; gap: var(--sp-2) var(--sp-3); align-items: center; padding: var(--sp-3); border: 1px solid var(--c-border); border-radius: var(--radius-md); background: var(--c-bg); min-width: 0; }
    .expression-settings-preview { width: 52px; height: 52px; display: flex; align-items: center; justify-content: center; overflow: hidden; }
    .expression-settings-preview--transparent { background: transparent; }
    .expression-settings-preview--square { background: color-mix(in srgb, var(--c-bg-hover) 85%, transparent); border: 1px solid var(--c-border); border-radius: var(--radius-sm); }
    .expression-settings-preview--circle { background: color-mix(in srgb, var(--c-bg-hover) 85%, transparent); border: 1px solid var(--c-border); border-radius: 999px; }
    .expression-settings-preview--accent { background: color-mix(in srgb, var(--c-accent) 10%, var(--c-bg)); border: 3px solid var(--c-accent); border-radius: var(--radius-sm); }
    .expression-settings-thumb { width: 100%; height: 100%; object-fit: contain; display: block; }
    .expression-settings-preview--circle .expression-settings-thumb { object-fit: cover; }
    .expression-settings-empty { width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; border: 1px dashed var(--c-border); border-radius: var(--radius-sm); color: var(--c-fg-muted); font-family: var(--font-mono, monospace); font-size: var(--font-size-xs); }
    .expression-settings-preview--circle .expression-settings-empty { border-radius: 999px; }
    .expression-settings-card-body { min-width: 0; }
    .expression-settings-label { color: var(--c-fg); font-size: var(--font-size-sm); font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .expression-settings-meta { margin-top: 2px; color: var(--c-fg-muted); font-size: var(--font-size-xs); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .expression-settings-actions { grid-column: 1 / -1; display: flex; flex-wrap: wrap; gap: var(--sp-2); }
  </style>

  <section class="theme-section">
    <div class="expression-settings-summary">
      <span><strong>${configuredCount}</strong> sprites configured</span>
      <span><strong>${settings.labels.length}</strong> expression slots</span>
    ...
    settings.spritesEnabled ? "checked" : ""
  }>
          <span class="toggle-slider"></span>
          <span class="toggle-text">Use sprite images</span>
        </label>
      </div>
      <div class="llm-field">
        <label for="expr-fallback-mode">Missing sprite fallback</label>
        <select id="expr-fallback-mode" class="input-field llm-input">
          <option value="label" ${
    settings.fallbackMode === "label" ? "selected" : ""
  }>Display emotion name</option>
          <option value="closest" ${
    settings.fallbackMode === "closest" ? "selected" : ""
  }>Use closest related sprite</option>
          <option value="none" ${
    settings.fallbackMode === "none" ? "selected" : ""
  }>Display nothing</option>
        </select>
      </div>
      <div class="llm-field">
        <label for="expr-frame-style">Sprite framing</label>
        <select id="expr-frame-style" class="input-field llm-input">
          <option value="transparent" ${
    settings.frameStyle === "transparent" ? "selected" : ""
  }>Transparent</option>
          <option value="square" ${
    settings.frameStyle === "square" ? "selected" : ""
  }>Soft square</option>
          <option value="circle" ${
    settings.frameStyle === "circle" ? "selected" : ""
  }>Circle crop</option>
          <option value="accent" ${
    ...
    settings.mobileSide === "right" ? "selected" : ""
      ...
      <div class="llm-field">
        <label class="toggle-label">
          <input type="checkbox" id="expr-show-subtitle" role="switch" aria-label="Show sprite subtitle" ${
    settings.showSubtitle ? "checked" : ""
  }>
          <span class="toggle-slider"></span>
          <span class="toggle-text">Show subtitle</span>
        </label>
      </div>
      <div class="llm-field">
        <label class="toggle-label">
          <input type="checkbox" id="expr-cleanup-checkerboard" role="switch" aria-label="Remove checkerboard backgrounds" ${
    settings.cleanupCheckerboardBackgrounds ? "checked" : ""
  }>
          <span class="toggle-slider"></span>
          <span class="toggle-text">Remove checkerboard backgrounds</span>
        </label>
      </div>
      <div class="llm-field">
        <button class="btn btn--primary" type="button" onclick="saveExpressionDisplaySettings()">Save</button>
      </div>
    </div>

    <div class="expression-settings-import">
      <div class="llm-field" style="flex:0 0 auto;">
        <label>Sprite pack ZIP</label>
        <label class="btn btn--sm" style="position:relative;overflow:hidden;cursor:pointer;">
          Choose ZIP
          <input type="file" id="expression-pack-file" accept=".zip,application/zip,application/x-zip-compressed" style="position:absolute;inset:0;opacity:0;cursor:pointer;" onchange="document.getElementById('expression-pack-file-name').textContent = this.files[0]?.name || ''">
        </label>
        <span id="expression-pack-file-name" class="anchor-meta"></span>
      </div>
      <button class="btn btn--primary" type="button" onclick="importExpressionSpritePack()">Import Pack</button>
    </div>
  </section>

  <section class="theme-section" style="margin-top:var(--sp-4);">
    <div class="expression-settings-grid">
      ${cards}
    </div>
  </section>`;
}
...
export function renderVoiceCallView(
  ...
  <div class="voice-call">
    ...
    <div class="voice-controls">
      ...
      <button class="voice-btn" id="voice-btn-screen" data-screen-presence-toggle onclick="Psycheros.toggleScreenPresence()" aria-label="Share screen" title="Share screen" aria-pressed="false">
