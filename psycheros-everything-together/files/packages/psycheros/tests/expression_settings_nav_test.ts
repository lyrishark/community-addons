...
Deno.test({
  name: "Everything Together stamps client assets for webview cache refresh",
  ...
  fn() {
    ...
    assertStringIncludes(
      html,
      "/css/main.css?v=everything-together-0.4.0-rc.3",
    );
    assertStringIncludes(
      html,
      "/js/psycheros.js?v=everything-together-0.4.0-rc.3",
    );
  },
});

Deno.test({
  name: "Everything Together does not offline-cache the app shell",
  ...
  async fn() {
    ...
    assertStringIncludes(
      sw,
      "psycheros-offline-__VERSION__-everything-together-0-4-0-rc-3",
    );
