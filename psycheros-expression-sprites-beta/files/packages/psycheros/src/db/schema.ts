...
function runMigrations(db: Database): void {
  ...
  const hasExpressionStateCol = db
    .prepare(
      "SELECT 1 FROM pragma_table_info('messages') WHERE name = 'expression_state'",
    )
    .get();

  if (!hasExpressionStateCol) {
    db.exec("ALTER TABLE messages ADD COLUMN expression_state TEXT");
