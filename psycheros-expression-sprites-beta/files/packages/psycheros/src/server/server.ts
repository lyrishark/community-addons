...
import {
  ...
  handleHealth,
  handleImportExpressionSpritePack,
...
export class Server {
  ...
  private async handleAPIRoute(
    ...
    // POST /api/expression-sprites/settings - Save expression display settings
    if (method === "POST" && path === "/api/expression-sprites/settings") {
      return await handleSaveExpressionDisplaySettings(ctx, request);
    }

    // POST /api/expression-sprites/import - Import a sprite pack ZIP
    if (method === "POST" && path === "/api/expression-sprites/import") {
      return await handleImportExpressionSpritePack(ctx, request);
    }

    // POST/DELETE /api/expression-sprites/:label - Manage one sprite slot
    const expressionSpriteMatch = path.match(
      /^\/api\/expression-sprites\/([^/]+)$/,
    );
    if (expressionSpriteMatch && expressionSpriteMatch[1] !== "settings") {
      if (method === "POST") {
        return await handleUploadExpressionSprite(
          ctx,
      ...
      if (method === "DELETE") {
        return await handleDeleteExpressionSprite(
          ctx,
          expressionSpriteMatch[1],
        );
      }
  ...
  private async handleStaticRoute(
    ...
    // GET /fragments/settings/vision/expressions - Expressions tab content
    if (path === "/fragments/settings/vision/expressions") {
      return await handleVisionExpressionsFragment(ctx);
    }
    ...
    // Serve expression sprites from .psycheros/expression-sprites/
    if (path.startsWith("/expression-sprites/")) {
      return await handleServeExpressionSprite(
        ctx,
        path.replace("/expression-sprites/", ""),
      );
    }
