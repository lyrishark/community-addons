...
resolve_psycheros_root() {
  ...
  if [ "${#matches[@]}" -gt 1 ]; then
    printf 'Multiple Psycheros source folders were found:\n' >&2
    printf '  - %s\n' "${matches[@]}" >&2
    die "Run again with ./install.sh /path/to/Psycheros"
  fi
...
if [ "$version" != "$supported_version" ]; then
  die "This add-on supports Psycheros $supported_version; found ${version:-unknown}. No files were changed."
fi
